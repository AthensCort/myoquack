function normalizeWaveformData(value) {
...
export function buildSessionExport(session, patient, events) {
    const serializedEvents = events.map(serializeEvent);
    const totalContraccionesEfectivas = events.filter((event) => event.peak_uv >= session.config_threshold_uv).length;
    return {
