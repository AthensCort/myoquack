...
function sortEventsByOrder<T extends { numero_orden: number }>(events: T[]) {
...
router.get("/:id/events/csv", async (req, res) => {
  try {
    ...
    const csv = buildEventsCsv(
      sortEventsByOrder(session.eventos).map((event) => serializeEvent(event)),
    );
  ...
  try {
    ...
    return res.json(
      buildSessionExport(
        session,
        session.paciente,
        sortEventsByOrder(session.eventos),
      ),
    );
  } catch (error) {
...
router.get("/", async (req, res) => {
  try {
    ...
    return res.json(
      sessions.map((session) => serializeSession(session, session._count.eventos)),
    );
  } catch (error) {
...
router.post("/", async (req, res) => {
  try {
    ...
    const reportName = String(
      req.body?.reportName ?? report?.nombre_reporte ?? "",
    ).trim();
    ...
    const createdSession = await prisma.sesionEMG.create({
      data: {
        ...
        eventos: {
          create: events.map((event: Record<string, unknown>, index: number) => ({
